...
export const paramDef = { type: 'object', properties: { title: { type: 'string', minLength: 1, maxLength: 200, pattern: '\\S' } }, required: ['title'] } as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(@Inject(DI.config) config: Config, liveStreamingService: LiveStreamingService) {
		super(meta, paramDef, async (ps, me) => {
			...
			try {
				const { stream, publishToken } = await liveStreamingService.create(me, ps.title.trim());
				return { stream: await liveStreamingService.pack(stream, me), publishUrl: `${config.liveStreaming.rtmpUrl}/${stream.mediaPath}?token=${publishToken}` };
			} catch (error) {
