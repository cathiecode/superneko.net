...
const en = {
	...
	guestUrl: 'Public guest URL', copyGuestUrl: 'Copy public URL', copyRtspUrl: 'Copy RTSP URL', share: 'Share Howl', sharePost: (title: string, url: string) => `${title} is live on Superneko. Watch it now.\n${url}`, end: 'End Howl', endConfirm: 'End this Howl?', ended: 'This Howl has ended', forceEnd: 'Force end Howl',
...
const ja = {
	...
	guestUrl: '外部視聴用URL', copyGuestUrl: '外部視聴用URLをコピー', copyRtspUrl: 'RTSP URLをコピー', share: 'Howlを共有', sharePost: (title: string, url: string) => `${title} をすーぱーねこで配信中。今すぐ見ろ。\n${url}`, end: 'Howlを終了', endConfirm: 'このHowlを終了しますか？', ended: 'Howlは終了しました', forceEnd: 'Howlを強制終了',
